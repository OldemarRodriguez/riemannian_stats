...
class TestVisualization(unittest.TestCase):
    ...
    def test_create_visualization_object(self):
        viz = Visualization(self.data, self.components, self.explained_inertia, self.clusters)
    ...
    def test_plot_2d_scatter_with_clusters(self):
        viz = Visualization(self.data, self.components, self.explained_inertia, self.clusters)
        try:
            viz.plot_2d_scatter_with_clusters("feature1", "feature2", "species", title="2D Scatter Test")
        except Exception as e:
    ...
    def test_plot_3d_scatter_with_clusters(self):
        viz = Visualization(self.data, self.components, self.explained_inertia, self.clusters)
        try:
            viz.plot_3d_scatter_with_clusters("feature1", "feature2", "feature3", "species", title="3D Scatter Test")
        except Exception as e:
    ...
    def test_plot_correlation_circle(self):
        ...
        viz = Visualization(self.data, self.components, self.explained_inertia)
        try:
            viz.plot_correlation_circle(self.correlations.T, title="Correlation Circle Test")
        except Exception as e:
